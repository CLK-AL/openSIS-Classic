...
if ($_REQUEST['modfunc'] === 'remove' && CSRFSecure::ValidateToken(optional_param('TOKEN', '', PARAM_RAW))) {
    ...
    if ($id > 0) DBQuery("DELETE FROM inventory_categories WHERE id=$id AND school_id='$school'");
}
if ($_REQUEST['modfunc'] === 'save' && CSRFSecure::ValidateToken(optional_param('TOKEN', '', PARAM_RAW))) {
    ...
    if ($title) DBQuery("INSERT INTO inventory_categories (school_id, title, sort_order, updated_by) VALUES ('$school', '$title', $sort, $staffId)");
}

$cats = DBGet(DBQuery("SELECT * FROM inventory_categories WHERE school_id='$school' ORDER BY sort_order, title"));
PopTable('header', 'Equipment Categories');
...
<?php
$columns = ['TITLE'=>'Category', 'SORT_ORDER'=>'Sort'];
